                        ...
                        if (!preg_match('/^[a-zA-z0-9_]+$/', $data->login)) {
                            ...
                            if (!empty($emailExist)) {/* (FR)si l'adresse existe on revois un message d'erreur  */
                                ...
                                if (!filter_var($data->email, FILTER_VALIDATE_EMAIL)) {
                                    ...
                                    $message->email .= " <a href='http://localhost/" . BASE_URL . "/users/confirmation/email:" . $SaveUser->email . "/key:" . $SaveUser->validatekey . "'>Comfirmer Votre email </a> ";
                                    SendMail::sendEmail($data->email, $message->email, 'Confirmation d\'inscription');
